...
"""
    sol2data(sol::AbstractSolution, s::ModelingToolkit.AbstractSystem, m::Model, p::Plan)

Construct an MVTSeries of the full solution from the MTK solution.
...
plan = Plan(m1, 1+m.maxlag:IDX-m.maxlead)
s = stacked_time_system(m, plan_data; fctype, plan)
nf = NonlinearFunction(s)
prob = NonlinearProblem(nf, zeros(length(unknowns(s))))
sol = solve(prob, NewtonRaphson())
result_mvts = sol2data(sol, s, m, plan)
```
"""
function sol2data end
export sol2data

...
To implement a derived class, one must specialize the following methods for the `sd` argument of the new type.
  * Constructor - normally it would take a model and a plan together with a final condition type.
  * `global_RJ(x, exog, sd)` - compute the residual and the Jacobian (return a 2-tuple) of the system at the given point `x` and exogenous data `exog`.
...
Compute the residual and the Jacobian of the dynamic system of equations at the given point `x` using the given exogenous data `exog`.
Returns a tuple of two elements. The first is a vector of residuals and the second is the Jacobian matrix.
"""
...
Apply the exogenous constraints according to the simulation plan (passed to the constructor of sd) with exogenous values taken from `exog`.
"""
...
Apply final conditions of the given type. The type parameter `T` must be one of the constants of type `FCType`. 
If `T` is fcgiven, the final condition values are taken from `exog`. If `T` is one of `fclevel` or `fcslope`, then the final conditions are taken from
the steady state solution in the model instance passed to the constructor of `sd`.
"""
